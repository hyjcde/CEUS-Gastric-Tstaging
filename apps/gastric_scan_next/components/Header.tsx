...
import { Globe, User, LogOut, FileText, BarChart2, PenTool, ScanSearch, History, KeyRound } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ onShowStatistics, selectedPatient }) => {
  ...
  const { account, logout } = useDoctorAccount();
    ...
    <header className="relative z-50 flex h-full w-full min-w-0 items-center justify-between gap-3 overflow-visible border-b border-white/10 bg-[#08090a] px-3 shadow-md sm:px-4">
      <div className="flex min-w-0 flex-1 items-center gap-3 lg:gap-5">
        ...
        <div className="relative hidden shrink-0 lg:block" ref={langRef}>
          <button
            ...
            title={language === 'en' ? 'Simplified / Traditional (HK) / EN' : '简体 / 香港繁體 / EN'}
          >
            <Globe size={10} />
          ...
          {showLangMenu && languageMenuPosition ? createPortal(
            ...
            >
              {([
                { id: 'zh' as const, label: language === 'en' ? 'Simplified Chinese' : '简体中文' },
                { id: 'zh-HK' as const, label: language === 'en' ? 'Traditional Chinese (HK)' : '繁體中文（香港）' },
      ...
      <div className="flex shrink-0 items-center gap-1.5 text-[10px] font-mono text-gray-500 sm:gap-2">
        ...
        <button
          ...
          className="flex items-center gap-2 rounded border border-white/5 bg-[#111] px-2 py-1.5 text-sky-300 transition-colors hover:border-sky-500/50 hover:bg-sky-500/10 hover:text-sky-200 sm:px-2.5"
          title={language === 'en' ? 'My history' : (language === 'zh-HK' ? '我的歷史' : '我的历史')}
        >
          <History size={12} />
          <span className="hidden text-[10px] font-semibold sm:inline">
            {language === 'en' ? 'History' : (language === 'zh-HK' ? '歷史' : '历史')}
          </span>
        </button>
        ...
        <div className="relative" ref={menuRef}>
          <button
            ...
            className="flex h-8 min-w-8 shrink-0 items-center justify-center rounded-full border border-white/10 bg-gradient-to-tr from-gray-800 to-gray-700 px-2 text-xs font-bold text-gray-300 shadow-lg transition-colors hover:border-blue-500/50"
          >
            {(account?.display_name || 'DR').slice(0, 2).toUpperCase()}
          </button>

          {showUserMenu && userMenuPosition ? createPortal(
            <div
              ref={userMenuRef}
              className="fixed z-[200000] flex w-56 flex-col rounded-lg border border-white/10 bg-[#111] py-1 shadow-2xl backdrop-blur-xl"
              style={userMenuPosition}
            >
              <div className="border-b border-white/5 px-4 py-3">
                <div className="text-xs font-bold text-white">
                  {account?.display_name || t.userMenu.name}
                </div>
                <div className="text-[10px] text-gray-500">
                  {account?.account_id || t.userMenu.role}
                </div>
              </div>
              ...
              >
                <KeyRound size={12} />
                {account
                  ? (language === 'en' ? 'Switch account' : (language === 'zh-HK' ? '切換帳號' : '切换账号'))
                  : (language === 'en' ? 'Sign in' : (language === 'zh-HK' ? '登入' : '登录'))}
              </button>
              <button
                onClick={() => {
                  setShowUserMenu(false);
              ...
              >
                <History size={12} />
                {language === 'en' ? 'My history' : (language === 'zh-HK' ? '我的歷史' : '我的历史')}
              </button>
              <button
              ...
              <button
                ...
                className="flex items-center gap-3 px-4 py-2 text-left text-[11px] text-red-400 transition-colors hover:bg-red-500/10 hover:text-red-300"
              >
