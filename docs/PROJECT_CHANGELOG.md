...
- Reason: The panel had too much Chinese copy. Keep short English labels in Times New Roman.
- Key changes: Title is case and time only. On-image tags are Mucosa / Muscularis / Serosa / Lost. Legend drops long notes. Clustering unchanged. Does not unlock cT.
- Validation: Offline P040 re-render.
- Deployment: none (offline figure).

## 2026-08-29, P040 keep muscularis inside the serosa strip

- Scope: `scripts/wall_lesion_aware_cluster.py` (strip order), `scripts/render_wall_layer_thin_bands.py --case P040`.
- Reason: Pinning outer dark pixels let the pink muscularis wash past the mint serosa. At the right-hand start the three belts must stay separate: inner bright, mid dark, outer bright.
- Key changes: Assign by gray inside each across strip. Muscularis stops at the inner edge of serosa. Overlay form unchanged. Does not unlock cT.
...
- Reason: The wash form was right, but bands stopped early (5 px exclude, 10 px display gap) and were sliced more by depth than by gray.
- Key changes: Seed on the right, then assign each station by pixel gray and walk left. Stop only on lesion pixels. Display gap is 1 px. Overlay style unchanged. Does not unlock cT.
- Validation: Synthetic cluster test; offline P040 re-render.
- Deployment: none (offline figure).
