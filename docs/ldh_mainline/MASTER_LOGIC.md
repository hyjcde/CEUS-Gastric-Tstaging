...
| 2026-06-12 | 手稿 v2.4 + G33 闭环 | Reader study 2-arm 视频优先重设计：子集 n=150 (Arm A 91 AI-clean + Arm B 59 AI-uncertain) 替换单臂设计；3 reader 含 site PI Dr. Zhuo (Fujian Med Univ Xiehe Hospital)；a priori 80% power 检 0.10 abs 提升 (α=0.05, ρ=0.5, baseline 0.65)；2-pass (no-AI / with-AI) 减 carry-over；极简阅片包 `apps/tstage_reader_study/` (4 选 1 T-stage + 0.25/0.5/1/2× 倍速 + scrubber) 替换原 5 选项 app；§Summary L75 + §Methods L201 + §Discussion L394 + §Limitations #3 全部更新；Appendix S10 §app:reader (L554) 完全重写。**主文 AI raw test-set 数字仍不报**（boss + 卓医生 微信 2026-06-12 共识：测试集 AI 太低，论文里不合适；先 AI 筛出相对不错的 150 例让人测）；子集算法准确率不直接代表全队列已在 §Limitations #3 明示 |
| 2026-06-12 | dc28480 (HTML 打磨) | tstage_reader_study UI 第二轮：进度条 N/150 实时；首次访问自动弹操作说明 (含 4 步 + 2-pass 解释)；键盘快捷键 1-4/Space/S/→/?；Pass 2 选完一选项停留 900 ms 显示与 AI 一致/不一致反馈；Pass 2 完成页加医生-AI-病理真值表（opt-in，避免偏倚）；README 补键盘表 + Pass 2 反馈 + 进度条 |
| 2026-06-12 | 第 4 波 + 临床规则 | 新增 `scripts/aggregate_reader_results.py` 收 3 reader 跑出的 JSON 产出 per-case CSV (450 行) + cross-reader MD/JSON（accuracy vs truth, agree_with_ai rate, paired AI uplift Pass 2−Pass 1, Cohen's κ）；完成页真值表改 opt-in 满足 `clinical-validation.md` 规则（"不展示患者临床信息/AI 判断/病理分期到阅片界面"，仅 AI 是 Pass 2 实验条件，病理仅完成自检可点开）；Appendix S10 §app:reader 同步 UI 描述（进度/弹窗/键盘/反馈/真值表 opt-in/聚合脚本）；SSOT_MAP / script_registry.csv 同步登记 |
