...
日期：2026-08-29。本文写的是**像素三类聚类对照**，不是当前主方法。当前主方法是有序细线追踪：[WALL_ORDERED_CURVE_TRACKING.md](./WALL_ORDERED_CURVE_TRACKING.md)。不定 cT。
...
一句话（对照路径）：医生画一条走行线。程序把走廊像素按整体灰度聚成亮 / 暗，再按法向深度拆开。这只能提供候选色块。最终分层以细线追踪为准。
