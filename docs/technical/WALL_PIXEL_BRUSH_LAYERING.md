...
一句话：医生画一条走行线（画笔中心线）。程序把线两侧一定宽度的像素收成一条走廊，挖掉病灶，再在走廊里**按图像列从上到下均分成三条互不重叠的带**。黄 / 红 / 绿就是这三条带，不是灰度 k-means 随便涂的色块。
...
y 最小的 i 个点     -> 标签 0  浅层 / 黄
中间 j-i 个点       -> 标签 1  固有肌 / 红
y 最大的剩下的点    -> 标签 2  浆膜 / 绿
```

同一列里标签只能是 `0, 0, ..., 1, 1, ..., 2, 2`，**禁止黄插在绿里面**。
...
所以最终不是「每列死板的人数三等分」，而是「三等分当种子，再沿 x 拉成两条平滑横线」。三条带在图上应是上黄、中红、下绿，中间亮、两边暗时，红带落在亮层上。
...
3. **均分三份不是解剖厚度。** 真实黏膜可能比肌层薄。当前为了黄绿泾渭分明，先保证空间互斥，再谈按亮暗交界微调。
