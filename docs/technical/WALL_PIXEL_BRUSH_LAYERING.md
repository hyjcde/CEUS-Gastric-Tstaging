...
一句话：医生画一条走行线（画笔中心线）。程序把线两侧一定宽度的像素收成一条走廊，挖掉病灶，再在走廊里**按每一列的亮暗灰度切成三条互不重叠的带**（分界可比灰度交界往外多 2 px）。黄 / 红 / 绿就是这三条带，不是人数均分，也不是整图灰度 k-means 乱涂。
...
`prefer_strips=True` 时：**灰度聚类、法向 across、腔侧翻转都不参与贴标签**。标签只由「同一图像列、从上到下三等分」决定。
...
**当前贴标签不用 `across` 的正负来决定谁是浅层。** 它只用来丢掉笔刷最外圈暗背景：`|across| > 0.88` 的点不参加三等分（像素太少时退回全部 `keep`）。
...
得分 >= 8 就用这条切，否则退回人数三等分
然后中段两端各往外扩 GRAY_CUT_PAD=2 个像素（把亮暗交界的过渡带收进中间层）
...
所以最终是「每列按灰度找亮暗交界，中段外扩 2 px，再沿 x 把两条分界抹顺」。三条带仍是上黄、中红、下绿。中间亮、两边暗时，红带应贴在亮层上，而不是占满整条走廊的三分之一。
...
3. **中段外扩 2 px 仍不是解剖厚度。** 真实黏膜可能更薄。分界跟着该列灰度走，对比度不够时才退回三等分。
