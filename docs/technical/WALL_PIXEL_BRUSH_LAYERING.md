...
一句话：医生画一条走行线（画笔中心线）。程序把线两侧一定宽度的像素收成一条走廊，挖掉病灶，再在走廊里**按每一列的亮暗灰度切成三条互不重叠的带**。找灰度带时，画笔上下各再看大约 12 px，这样薄回声不会被画笔边框裁掉。黄 / 红 / 绿就是这三条带，不是人数均分，也不是整图灰度 k-means 乱涂。
...
`prefer_strips=True` 时：**灰度聚类、法向 across、腔侧翻转都不参与贴标签**。标签只由「同一图像列的灰度剖面」决定，不是人数三等分。
...
**当前贴标签不用 `across` 的正负来决定谁是浅层。** 它只用来丢掉笔刷最外圈暗背景：`|across| > 0.92` 的点不参加切层（像素太少时退回全部 `keep`）。找灰度带时另在画笔上下各读约 12 px。

`along_idx` 当前主路径也不切层，只留在 `ClusterArm` 里给旧 fate / 中心线函数用。

---

## 6. 当前贴标签：`assign_natural_y_bands`

这是 2026-08-29 的像素分层核心。设计原因：两层暗回声灰度很像，先按灰度 k-means 会把黄和绿合成一类，再涂到图上就会夹杂。

### 6.1 核心思想（用列当尺子）

把走廊看成许多竖条（同一 `x`）。每一条从上到下（`y` 增大）是墙的厚度方向（走行线大致水平时成立）。

在一条竖条里，把有效像素按 `y` 排序，看这一列的灰度剖面，找出「中间一条和其他两条不一样」的切法：

```text
对每个候选 (i, j)：
    上段 mean(gray[0:i])    中段 mean(gray[i:j])    下段 mean(gray[j:])
    得分 = max(中亮两侧暗, 中暗两侧亮)

先在画笔 y 范围上下各扩 GRAY_SEARCH_PAD=12 px 读这一列灰度
阈值用画笔里的像素算，避免腔或远场把门槛拉歪
再找与画笔重叠的最亮或最暗连续段（整条走廊多数列锁定同一种：中亮或中暗）
切完两端最多再外扩 GRAY_CUT_PAD=12 px，让中段盖住整条灰带，两侧至少各留 2 px
对比太弱才退回人数三等分
...
同一列里标签只能是 `0, 0, ..., 1, 1, ..., 2, 2`，**禁止黄插在绿里面**。中间层可以比两侧薄或厚，不再是均匀三分之一。
...
```text
core = keep AND |across| <= 0.92
若 core 点数 < 40，则 core = keep
一列里 core 点数 < 4，该列暂不贡献分界
至少要有 3 列成功，否则整图不贴标签（全 -1）
```

外圈 `|across| > 0.92` 且偏暗的，多半是笔刷沾到的腔或远处暗背景，不当第一层。找灰度时仍可在画笔外约 12 px 读图。

### 6.3 左右抹顺，避免每列各切各的

每列先得到两个分界纵坐标 `y1`（0/1）和 `y2`（1/2）。然后：

1. 对 `y1(x)`、`y2(x)` 做窗口至少 15 的一维中值
2. 再做半径 3 的三角核平滑
3. 强制 `y2 >= y1 + 1`
4. 缺列用邻列插值
5. **用抹顺后的 `y1(x)`、`y2(x)` 重新给该列每个像素贴 0/1/2**

所以最终是「每列按灰度找亮暗交界，中段最多外扩 12 px 盖住灰带，再沿 x 把两条分界抹顺」。三条带仍是上黄、中红、下绿。中间亮、两边暗时，红带应贴在亮层上，而不是占满整条走廊的三分之一。

### 6.4 病灶像素

`assign_lesion=False`（出图）：灶内保持 `-1`，图上不涂色。  
`assign_lesion=True`：灶内也按同一套 `y1(x)`、`y2(x)` 切开，仅供旧实验。

`fit_side` 和 `features` 在这条路径里被丢掉，不参与切层。
...
| 灰度搜索外扩 | 12 px | 画笔上下各再读一截，避免薄回声被裁掉 |
