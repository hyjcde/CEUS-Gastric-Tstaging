...
工作台对象只回答「在哪画、分几层、清不清楚」。**2026-08-29 口径：聚类和 DINO 只提供候选点或候选带，最终分层是病灶感知的有序细线追踪**，见 [WALL_ORDERED_CURVE_TRACKING.md](../technical/WALL_ORDERED_CURVE_TRACKING.md)。不能靠平行偏移冒充层次，也不能要求 DINO 直接吐出三条干净色带。过门以后写进现有 1/2/3 线和四档芯片，不定 cT。
