    ...
    path.write_text(
        "\n".join(
            [
                ...
                "  5. 可选：点「绑定自动保存」，筛图后自动写入 gradcam_review_sync.json",
                "",
                "【恢复历史进度 · 两步即可】",
                "  1. 把 CSV 复制到本文件夹（与 HTML 同级）",
                "     推荐文件名：gradcam_rejected.csv 或 gradcam_review_sync.csv",
                "  2. 打开页面后点「同步文件夹中的 CSV」",
                "  页面启动时也会自动读取上述 CSV（若存在）",
                "",
                "【记录同步 · 不会随便覆盖】",
                "  同步时：本机较新的记录优先保留，只补充缺失条目",
                "  筛图过程中可绑定 gradcam_review_sync.json 自动保存",
                "  兼容 CSV：gradcam_review_export.csv / 筛图记录.csv 等",
                "",
                "【快捷键】",
