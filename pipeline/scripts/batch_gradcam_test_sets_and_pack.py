    ...
    path.write_text(
        "\n".join(
            [
                ...
                "  3. 默认「简易模式」：只看图像质量，不用管 AI 预测",
                "  4. 质量差的图点「✕ 质量差，剔除」或按 X；质量好的点「✓ 保留」或按 K",
                "  5. 操作后自动跳下一张未浏览；顶部进度条显示完成情况",
                "  6. 筛完后点「导出剔除 CSV」，发回算法组",
                "",
                "【快捷键】",
                "  → 下一张   X 剔除   K 保留   Z 撤销   F 全屏",
                "  1/2/3 快选剔除原因（层次不清 / 伪影 / 其他）",
                "",
                "【专家模式】",
                "  左侧可切换「专家模式」，查看 AI 预测概率与误分标注（算法组用）",
                "",
            ]
        )
        + "\n",
