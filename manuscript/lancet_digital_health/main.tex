...
Figure~\ref{fig:pipeline} summarises the staged pipeline. \textbf{Localisation:} YOLO11- and RT-DETR-style detectors were trained with centre-aware augmentation; we prioritised recall and ROI success because missed lesions triggered centre-crop fallback. \textbf{Segmentation:} U-Net variants (ConvNeXt-Tiny encoder) were trained under patient-level constraints; predicted masks ($\approx$12\,193 images) supported ROI construction and mask4ch reference classifiers. Alternative segmenters (SAM2 cascade, nnU-Net variants) were explored in the supplementary appendix.
...
Training used class rebalancing and standard augmentation. Table~\ref{tab:clinical_harm} summarises why under-staging T2 or T3 carries higher clinical risk than over-staging in selected pathways; cost-sensitive thresholds are prespecified but not yet locked for deployment. Hyperparameters were selected on the validation split only.
...
\input{figures/study_flow}
\input{figures/pipeline_diagram}

\subsection{Cohort and label characteristics}
Table~\ref{tab:cohort} and Table~\ref{tab:tstage_dist} summarise splits and T-category imbalance. Masks were available for more than 95\% of training and validation frames. Video archives (2018--2025) exist for future video multiple-instance learning but were outside the primary inference endpoint of this report.

\input{tables/cohort_tstage}
\input{tables/clinical_harm}
...
Qualitative review indicated T2$\rightarrow$T3 confusion clustered where ROI generation failed or predicted segmentation Dice was low. Contrast-agent subtype and acquisition-year shifts contributed to external degradation. Confusion matrices, ROI-fallback rates, and Grad-CAM atlases are provided in the supplementary appendix and project visualisation repository (\texttt{results/visualizations/}).
...
See \texttt{supplementary\_appendix.pdf} for detector ablations, extended segmentation comparisons (SAM2 cascade, SAM3 zero-shot, nnU-Net), per-class Grad-CAM correlations, AI reporting checklist items, and reproducibility paths.
