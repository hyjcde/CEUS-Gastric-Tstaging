...
病灶感知分层离线 A/B（先排除病灶再聚亮-暗-亮，不定 cT）：`wall_lesion_aware_cluster.py`、`pack_wall_layer_fixture_v1.py`、`eval_lesion_aware_wall_cluster_v1.py`、`render_lesion_aware_wall_cluster_panel.py`、`test_wall_lesion_aware_cluster.py`。pack 默认读公网 ZML 走行线并按画线时间抽帧。计划见 `docs/plans/LESION_AWARE_WALL_CLUSTER_20260828.md`。
