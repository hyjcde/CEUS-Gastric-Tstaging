...
病灶感知分层离线 A/B（先排除病灶再聚亮-暗-亮，不定 cT）：`wall_lesion_aware_cluster.py`、`pack_wall_layer_fixture_v1.py`、`eval_lesion_aware_wall_cluster_v1.py`、`eval_lesion_aware_wall_cluster_trad.py`、`render_lesion_aware_wall_cluster_panel.py`、`test_wall_lesion_aware_cluster.py`。传统对照：k-means / GMM / Ward / FCM / 一维灰度 / 一维法向。诊断拼图：`render_wall_cluster_diagnostics.py`（胃腔朝向、灰度-深度散点、翻转剖面）。记录图 + 黄线内三层细块：`render_wall_layer_thin_bands.py`（先裁紧 ROI 再分割蓝色病灶，再放大灶周三层，不画统计图）。当前主方法是病灶外有序细线追踪加单帧浸润判定（不定 cT）：`wall_ordered_curve_track.py`、`wall_invasion_readout.py`、`render_wall_ordered_curves.py`、`test_wall_ordered_curve_track.py`，说明见 `docs/technical/WALL_ORDERED_CURVE_TRACKING.md`。像素三类聚类只做对照：`render_wall_pixel_vs_dino_cluster.py`。pack 默认读公网 ZML 走行线并按画线时间抽帧。计划见 `docs/plans/LESION_AWARE_WALL_CLUSTER_20260828.md`。
