...
| 标准医院名称 | 物理目录 | T 分期 manifest | 建模 split | 帧数 | 患者 | 良/恶性 | 视频/图片 | 状态 |
| --- | --- | --- | --- | ---: | ---: | --- | --- | --- |
| 福建医科大学附属协和医院 | `internal/` | 是 | train / val / test_external / test_prospective | 10,659 | 1,883 | 有/有 | 有/有 | 已入库 |
| 莆田学院附属医院 | `莆田学院附属医院/` | 是 | train / val / test_external | 2,376 | 344 | 有/有 | 有/有 | 已入库 |
| 莆田市第一医院 | `莆田市第一医院/` | 是 | test_external | 25 | 8 | 无/有 | 无/有 | 已入库 |
| 福建省肿瘤医院 | `福建省肿瘤医院/` | 是 | test_external | 436 | 135 | 有/有 | 无/有 | 已入库 |
| 三明市第二医院 | `三明市第二医院/` | 是 | test_external | 19 | 4 | 有/有 | 无/有 | 已入库 |
| 福建省德化县医院 | `福建省德化县医院/` | newzip | test_external_newzip | 100 | 16 | 有/有 | 有/有 | 已入库 |
| 北京友谊医院 | `北京友谊医院/` | newzip | test_external_newzip | 124 | 22 | 无/有 | 无/有 | 已入库 |
| 佛山市第一人民医院 | `佛山市第一人民医院/` | newzip | test_external_newzip | 112 | 31 | 无/有 | 有/有 | 已入库 |
| 湖北中西医结合医院 | `湖北中西医结合医院/` | newzip | test_external_newzip | 216 | 67 | 有/无 | 无/有 | 已入库 |
| 中核五〇四医院 | `data/中核五〇四医院/` | 否 | 独立溃疡任务 | 0 | 0 | 有/有 | 有/有 | 独立任务 |
...
| 旧目录名 | 现目录名 |
| --- | --- |
| 协和内部 | `dataset/internal/`（临床字段改为福建医科大学附属协和医院） |
| 莆田1胃癌直接手术 | 莆田学院附属医院 |
| 莆田2胃癌直接手术 | 莆田市第一医院 |
| 肿瘤医院直接手术 | 福建省肿瘤医院 |
| 三明胃癌直接手术 | 三明市第二医院 |
...
| 外省整理/湖北窦 | 湖北中西医结合医院 |
| 德化直接手术 | 福建省德化县医院 |
| 窦晓霞中核五〇四医院胃溃疡病例(1) | 中核五〇四医院 |

### newzip 各中心统计

`外省整理.zip` 已拆成三个独立物理目录：

| 标准医院名称 | 物理目录 | manifest 帧数 | 建模 split 帧数 | 建模 patient 数 |
| --- | --- | ---: | ---: | ---: |
| 北京友谊医院 | `北京友谊医院/` | 124 | 124 | 22 |
| 佛山市第一人民医院 | `佛山市第一人民医院/` | 112 | 67 | 31 |
| 湖北中西医结合医院 | `湖北中西医结合医院/` | 216 | 216 | 67 |
| 福建省德化县医院 | `福建省德化县医院/` | 100 | 88 | 16 |

说明：newzip 各中心在 modeling CSV 中使用独立 `source`，例如 `ext/newzip/北京友谊医院`。
...
| 胃溃疡视频良恶性口径                     | `data/中核五〇四医院/`                                                     | 76 个病例文件夹 / 158 个视频 / 1,990 个 1fps 帧 | 1,990 帧 | 良性 37 / 恶性 39 个病例文件夹  | 暂无统一 lesion mask                             | 良恶性溃疡视频泛化、筛查和帧级 pipeline review                      |
...
| 胃溃疡视频数据      | `data/中核五〇四医院/`                 | 良性 37 个病例文件夹、32 个视频、1fps 抽帧 452 帧；恶性 39 个病例文件夹、126 个视频、1fps 抽帧 1,538 帧 | 目录级良/恶性标签；暂未整理成统一 lesion mask 或 T-stage 标签 | 良恶性视频分类、pipeline 泛化 review、良性对照帧抽样 |
...
- 预处理输出：`dataset/external/{北京友谊医院,佛山市第一人民医院,湖北中西医结合医院,福建省德化县医院}/`
